...
interface ProductGridProps {
  limit?: number
  category?: string
  /** Optional SSR/Server Component preloaded products to improve LCP */
  initialProducts?: Product[]
  /** The page number for initialProducts (default: 1) */
  initialPage?: number
  /** Whether there are more products available after initialProducts */
  initialHasMore?: boolean
}

export function ProductGrid({ limit = 12, category, initialProducts, initialPage = 1, initialHasMore }: ProductGridProps) {
  ...
  useEffect(() => {
    if (!initialLoadDone.current) {
      ...
      // If we were given SSR/initial products, use them immediately to avoid a slow first paint.
      if (initialProducts && initialProducts.length > 0) {
        ...
        setHasMore(typeof initialHasMore === "boolean" ? initialHasMore : initialProducts.length >= limit)
        return
      }
