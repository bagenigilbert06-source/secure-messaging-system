...
export function TrendingNow({ initialProducts }: InitialProductsProps) {
  // Ensure hook return is typed so map callbacks get Product instead of implicit any.
  const { trending: trendingRaw, isLoading, hasCachedData, mutate } = useSWRTrending(initialProducts ? { fallbackData: initialProducts } : undefined) as {
    trending?: Product[]
    isLoading: boolean
