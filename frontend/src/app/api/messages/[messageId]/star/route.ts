...
export async function POST(
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${FLASK_API}/api/messages/${messageId}/star`,
      {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json({ success: true, is_starred: data.is_starred || false });
  } catch (error) {
    console.error('[API] Error starring message:', error);
    return NextResponse.json(
